...
export default function FindMarketersPage() {
  ...
  const hasIdea = Boolean(selectedIdea && productName);
  ...
  }, [hasIdea, selectedIdea, productName]);

  useEffect(() => {
    if (hasIdea) fetchMarketers();
  }, [hasIdea, fetchMarketers]);
    ...
    try {
      ...
      setDrafts((prev) => ({
        ...
        [index]: {
          ...
          body: data.error ? `Error: ${data.error}` : data.body || "",
          generating: false,
          generated: true,
        },
      }));
  ...
  const sendOutreach = (index: number) => {
    setDrafts((prev) => ({
      ...prev,
  ...
  return (
    <main className="bg-background min-h-screen">
      ...
      <div className="max-w-[1200px] mx-auto pt-32 pb-16 px-6">
        ...
        {!hasIdea && (
          <div className="border border-border rounded-[4px] p-8 mb-8">
            <p className="font-body font-light text-body text-foreground mb-3">
              Pick an idea first.
            </p>
            <p className="font-body font-light text-small text-muted mb-6 max-w-[520px]">
              Marketer matching uses your selected idea and product name to score
              relevance. Start with the idea generator, then come back here.
            </p>
            <div className="flex gap-3">
              <GhostButton href="/ideas">Generate ideas</GhostButton>
              <GhostButton href="/onboarding">Start onboarding</GhostButton>
            </div>
          </div>
        )}
        ...
        {hasIdea && !loading && marketers.length > 0 && (
          <motion.div
          ...
          >
            {marketers.map((m, i) => (
              <MarketerCard
                ...
                className={selected.has(i) ? "border-accent" : undefined}
              />
            ))}
          </motion.div>
        )}
